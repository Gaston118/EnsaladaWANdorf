...
client.on('connect', () => {
  console.log('✅ RECEPTOR 1 conectado y escuchando en "lan/broadcast/#"');
  
  // --- LÓGICA DE ESTE CLIENTE ---
  client.subscribe('lan/broadcast/#', (err) => {
    if (err) {
      console.error('Error al suscribir (Receptor 1):', err);
    }
  });
});

client.on('message', (topic, message) => {
  console.log(`\n=== 📥 Mensaje recibido en RECEPTOR 1 ===`);
...
client.on('error', (err) => {
  console.error('Error de conexión (Receptor 1):', err);
});